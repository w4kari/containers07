...
Приложение успешно отобразило PHP-страницу со списком транзакций, фильтрацией записей и итоговой суммой.
...
Явный порядок запуска между `frontend`, `backend` и `database` не задан, потому что в `docker-compose.yml` отсутствует директива `depends_on`. То есть, Compose создает и запускает контейнеры без строгой гарантии очередности готовности сервисов.
